...
from alkahest.epub_accessibility import (
    EpubPolicyError,
    finalize_epub,
    validate_epub,
)
...
def main():
    with tempfile.TemporaryDirectory(prefix="alkahest-epub-accessibility.") as temp:
        ...
        finalize_epub(reduced_epub, reduced_policy_path, allow_missing_sections=True)
        ...
        cases = [
            ...
            (
                "math-annotation",
                "EPUB/chapter.xhtml",
                replace('<annotation encoding="application/x-tex">x</annotation>', ""),
        ...
        fail_report.write_text(json.dumps(ace_report(failure=True)) + "\n", encoding="utf-8")
        run_ace_report_fixture(fail_report, False)
        drift_report = root / "ace-drift.json"
        drift_report.write_text(json.dumps(ace_report(version="latest")) + "\n", encoding="utf-8")
