...
def expect_artifact_failure(name, expected, mutate):
    ...
    with tempfile.TemporaryDirectory(prefix=f"alkahest-cover-{name}.") as temporary:
        ...
        try:
            ...
            raise RuntimeError(f"error: cover artifact fixture {name} unexpectedly passed")


def main():
    context = validate_cover_document(BASE_POLICY, RECORDS, PUBLICATION)
    geometry = cover_geometry(context["template"], RECORDS["print-full-7x10-en"]["dimensions"], 73)
    if (
    ...
    cases = [
        ...
        (
            "printer-id",
            "printer template id",
            lambda p, _r, _m: p["template"]["printer_template"].update(id="Bad ID"),
        ),
        (
            "revision",
            "revision must be",
            lambda p, _r, _m: p["template"]["printer_template"].update(revision=0),
        ),
        (
            "binding",
            "currently implements perfect-bound",
            lambda p, _r, _m: p["template"].update(binding="case-bound"),
        ),
        (
            "caliper",
            "outside its allowed range",
            lambda p, _r, _m: p["template"]["paper"].update(sheet_caliper_in="0"),
        ),
        (
            "page-policy",
            "must be round-up-even",
            lambda p, _r, _m: p["template"].update(page_count_policy="literal"),
        ),
        (
            "press-ready",
            "cannot claim press readiness",
            lambda p, _r, _m: p["template"].update(press_ready=True),
        ),
        (
            "barcode",
            "does not fit",
        ...
        (
            ...
            lambda p, _r, _m: p["profiles"][1].update(id=p["profiles"][0]["id"]),
        ),
        (
            "unknown-print",
            "needs a print manifestation",
            lambda p, _r, _m: p["profiles"][0].update(manifestation="missing"),
        ),
        (
            "relation",
            "differs from its print interior relation",
            lambda p, _r, _m: p["profiles"][0].update(interior_manifestation="pdf-full-6x9-en"),
        ),
    ...
    print("ok: cover fixtures (valid geometry/output; 14 policy and 5 artifact failures rejected)")
