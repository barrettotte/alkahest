...
BASE = json.loads((ROOT / "config/template/extension-apis.json").read_text(encoding="utf-8"))
...
def main():
    ...
    expect_failure("version", "semantic versioning", lambda value: value.update(api_version="next"))
    expect_failure("missing-api", "IDs must be exactly", lambda value: value["entries"].pop())
    expect_failure(
        "authority",
        "invalid authority",
    ...
    expect_failure(
        "missing-path",
        "path does not exist",
        lambda value: entry(value, "components")["entrypoints"].append("book/theme/missing.scss"),
    )
    expect_failure(
    ...
    expect_failure(
        "generator-task-coverage",
        "does not exactly cover deterministic generator tasks",
        lambda value: entry(value, "generators")["tasks"].pop(),
    )
    expect_failure(
        "generator-implementation-coverage",
        "does not exactly cover deterministic generator implementations",
    ...
    print(
        ...
        "(closed inventory and exact docs; 10 schema, path, and coverage failures rejected)"
    )
