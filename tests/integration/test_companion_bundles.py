...
def expect_failure(name, expected, mutate):
    with tempfile.TemporaryDirectory(prefix=f"alkahest-companion-bundle-{name}.") as temporary:
        ...
        try:
            ...
            if expected not in str(error):
                raise RuntimeError(
                    f"error: companion bundle fixture {name} missed {expected!r}: {error}"
                ) from error
        else:
            raise RuntimeError(f"error: companion bundle fixture {name} unexpectedly passed")


def main():
    with tempfile.TemporaryDirectory(prefix="alkahest-companion-bundle-valid.") as temporary:
        parent = Path(temporary)
    ...
    expect_failure(
        "archive-drift",
        "stale or changed",
        lambda root: (root / archive).write_bytes((root / archive).read_bytes() + b"changed"),
    )
    expect_failure(
        "sidecar-drift",
        "stale or changed",
