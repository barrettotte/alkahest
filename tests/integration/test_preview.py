    ...
    selected = [
        {
            ...
            "path": "index.qmd",
            "title": "Preface",
            "identifier": "sec-preface",
            "private": False,
        },
        {
            "source_id": "reference",
            "path": "reference.qmd",
            "title": "Core feature specimen",
            "identifier": "sec-core-feature-specimen",
            "private": False,
        },
    ]
    omitted = [
        {
            ...
            "identifier": "sec-math-system",
            "private": False,
        },
        {
            "source_id": "private-notes",
            "path": "private/notes.qmd",
            "title": "Private notes",
            "identifier": "sec-private-notes",
            "private": True,
        },
    ]
    html = (
...
def expect_failure(name, expected, mutate):
    ...
    try:
        ...
        if expected not in str(error):
            raise RuntimeError(
                f"error: preview artifact fixture {name} missed diagnostic {expected!r}: {error}"
            ) from error
    else:
...
def main():
    ...
    expect_failure(
        "extra-page", "HTML page allowlist differs", lambda s: s["html_pages"].add("math.html")
    )
    expect_failure(
        "omitted-id",
        "omitted source math",
        lambda s: s.__setitem__("html_text", s["html_text"] + " sec-math-system"),
    )
    expect_failure(
        "private-path", "private path", lambda s: s["html_entries"].add("private/notes.html")
    )
    expect_failure(
        "epub-identity",
        "EPUB contract",
        lambda s: s.__setitem__(
            "epub_text", s["epub_text"].replace(PREVIEW_UUID, "urn:uuid:wrong")
        ),
    )
    expect_failure(
        "citation",
        "ref-turing1936",
        lambda s: s.__setitem__(
            "html_text", s["html_text"].replace("ref-turing1936", "missing-citation")
        ),
    )
    expect_failure(
        "cross-reference",
    ...
    expect_failure(
        "pdf-trim",
        "PDF trim",
        lambda s: s.__setitem__("pdf_info", s["pdf_info"].replace("504 x 720", "432 x 648")),
    )
    expect_failure(
        "watermark",
        "PDF watermark source",
        lambda s: s.__setitem__("typ_text", s["typ_text"].replace(")[PREVIEW]", ")[DRAFT]")),
    )
