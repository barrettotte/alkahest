...
from alkahest.manifestations import (
    ContractError,
    validate_identifier,
    validate_record,
    validate_repository,
...
def main():
    ...
    expect_failure(
        ...
        lambda: validate_schema(
            changed(
                SCHEMA,
                lambda schema: schema["$defs"]["identifier"]["properties"]["scheme"]["enum"].append(
                    "untyped"
                ),
            )
        ),
    ...
    cases = [
        ...
        (
            ...
            lambda registry: manifestation(registry, "print-full-7x10-en")["production"].update(
                {"artifact": "book/_build/print/final.pdf"}
            ),
        ),
        (
        ...
        (
            ...
            lambda registry: manifestation(registry, "web-full-en")["availability"].update(
                {"status": "available"}
            ),
        ),
        (
            ...
            lambda registry: manifestation(registry, "web-preview-en").update({"relation": None}),
        ),
        (
        ...
        (
            ...
            lambda registry: manifestation(registry, "pdf-full-6x9-en")["production"].update(
                {"artifact": manifestation(registry, "pdf-full-7x10-en")["production"]["artifact"]}
            ),
        ),
        (
            "unsafe-artifact",
    ...
    with tempfile.TemporaryDirectory(prefix="alkahest-manifestation-cover.") as directory:
        ...
        with_cover = changed(
            REGISTRY,
            lambda registry: manifestation(registry, "web-full-en").update({"cover": cover}),
        )
