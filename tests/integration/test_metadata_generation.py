...
def main():
    ...
    expect_failure(
        ...
        lambda: validate_policy(
            changed(
                policy,
                lambda value: value["onix"]["contributor_roles"]["author"].update({"code": "A02"}),
            )
        ),
    )
    expect_failure(
    ...
    epub["identifiers"] = [item for item in epub["identifiers"] if item["scheme"] != "isbn-13"] + [
        {"scheme": "isbn-10", "value": "0306406152"}
    ]
    _, isbn10_status = output_bundle(retail_publication, isbn10_only, reproducibility, policy)
