...
def copy_fixture(root):
    ...
    for component in policy["directory_components"]:
        ignore = (
            shutil.ignore_patterns("fonts")
            if component["source"] == "book/theme"
            else None
        )
        shutil.copytree(
            ROOT / component["source"], root / component["source"], ignore=ignore
        )
...
def artifact_failure(name, expected, mutate):
    ...
    with tempfile.TemporaryDirectory(prefix=f"alkahest-template-output-{name}.") as temporary:
        ...
        expect_failure(
            name,
            expected,
            lambda: check_template_package(
                ROOT, output_root=output_root, extract=False
            ),
        )
...
def unsafe_archive():
    ...
    timestamp = datetime.strptime(source_date, "%Y-%m-%dT%H:%M:%SZ").replace(
        tzinfo=timezone.utc
    ).timetuple()[:6]
    output = io.BytesIO()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="alkahest-template-valid.") as temporary:
        ...
        if result["source_files"] != 62 or result["members"] != 64:
    ...
    policy_failure(
        ...
        lambda root: edit_policy(
            root, lambda value: value["package"].update(version="next")
        ),
    )
    policy_failure(
        "output",
        "must remain under",
        lambda root: edit_policy(
            root, lambda value: value["package"].update(output_root="dist")
        ),
    )
    policy_failure(
        "duplicate-component",
    ...
    policy_failure(
        ...
        "is not packaged",
        lambda root: edit_policy(
            root, lambda value: value["required_paths"].append("missing.txt")
        ),
    )
    policy_failure(
        "specimen-leak",
