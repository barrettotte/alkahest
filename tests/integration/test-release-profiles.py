...
DEFAULTS = json.loads(
    (ROOT / "book/alkahest-release-defaults.json").read_text(encoding="utf-8")
)
LOCAL = {
    "schema_version": 1,
    "sources": {
        ...
        "appendix": {"path": "appendices/checklist.qmd", "role": "appendix", "availability": "release"},
...
def document_failure(name, expected, mutate_defaults=None, mutate_local=None):
    ...
    if mutate_local:
        mutate_local(local)
    expect_failure(
        name, expected, lambda: release_outputs(encoded(defaults), encoded(local))
    )
...
if __name__ == "__main__":
    try:
        main()
    except (OSError, ReleaseProfileError, RuntimeError, UnicodeError) as error:
