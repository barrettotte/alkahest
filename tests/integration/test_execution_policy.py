...
VALIDATOR = ("-m", "alkahest.checks.execution_policy")
...
def main():
    with tempfile.TemporaryDirectory(prefix="alkahest-execution-tests.") as directory:
        ...
        expect_failure(
            ...
            lambda root: (
                json.loads((root / "editions.json").read_text(encoding="utf-8"))
        ...
        expect_failure(
            ...
            lambda root: replace(
                root / "execution-policy.json", '"release": "static-only"', '"release": "allowed"'
            ),
        )
        expect_failure(
            parent,
