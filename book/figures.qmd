...
WaveDrom 3.5.0 is retained as a serious evaluation candidate in
`figures/candidates/read-cycle-wavedrom.json` and
`figures/candidates/instruction-word-wavedrom.json`. Its compact WaveJSON and
register-field syntax are attractive, but normal builds do not yet carry a
pinned offline WaveDrom renderer. Mermaid and Graphviz remain appropriate for
automatically laid-out relationships; these bit- and timing-sensitive fixtures
instead use explicit geometry, selectable labels, fixed view boxes, embedded
descriptions, and byte-checked regeneration.
