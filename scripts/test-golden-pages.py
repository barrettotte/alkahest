...
POLICY = json.loads(
    (ROOT / "config/pdf/golden-pages.json").read_text(encoding="utf-8")
)
...
def main():
    ...
    expect_failure(
        ...
        lambda: validate_policy(
            changed_policy(
                lambda policy: policy["profiles"][1].__setitem__("backend", "typst")
            )
        ),
    )
    expect_failure(
    ...
    expect_failure(
        ...
        lambda: validate_policy(
            changed_policy(
                lambda policy: policy["pages"][0].__setitem__("rationale", "Too short")
            )
        ),
    )
