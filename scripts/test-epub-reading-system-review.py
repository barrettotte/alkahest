...
from lib.alkahest.epub_review import canonical_epub_sha256


ROOT = Path(__file__).resolve().parent.parent
CHECKER = ROOT / "scripts" / "check-epub-reading-system-review.py"
...
def write_fixture_epub(path):
    ...
    identifiers = [
        location["target_id"] for location in review["representative_locations"]
    ]
...
def main():
    with tempfile.TemporaryDirectory(prefix="alkahest-epub-review.") as temp:
        ...
        expect_failure(
            ...
            mutate_review(
                lambda value: value["reading_systems"][0]["scale_results"].pop()
            ),
        )
        expect_failure(
            parent,
        ...
        expect_failure(
            ...
            mutate_review(
                lambda value: value["artifact"].update(source_revision="a" * 40)
            ),
        )
        ...
        expect_failure(
            ...
            mutate_review(
                lambda value: value["claim"].update(conformance_claim=True)
            ),
        )
        expect_failure(
            parent,
            ...
            mutate_review(
                lambda value: value["claim"].update(status="reviewed-no-claim")
            ),
        )
    ...
    print(
        ...
        "(pending, partial, and conformant ledgers; timestamp-stable digest; "
