...
def check_decision(data):
    ...
    if decision["compatibility_backend"] not in BACKENDS:
        fail("compatibility PDF backend is unknown")
    if decision["default"] == decision["compatibility_backend"]:
        fail("default and compatibility PDF backends must differ")
    ...
    for backend in BACKENDS:
        expected_status = "default" if backend == default else "supported-compatibility"
...
def check_measurement(data):
    ...
    toolchain = (ROOT / "scripts/lib/toolchain.sh").read_text(encoding="utf-8")
...
def check_scores(data):
    ...
    if calculated[data["decision"]["default"]] < calculated[data["decision"]["compatibility_backend"]]:
        fail("default backend scores below the compatibility backend")
...
def check_repository_integration(data):
    ...
    for source in sorted(BOOK.rglob("*.qmd")):
        if raw_backend.search(source.read_text(encoding="utf-8")):
            fail(f"backend-specific raw markup leaked into manuscript source: {source.relative_to(ROOT)}")
...
def main():
    ...
    print(
        ...
        f"LuaLaTeX compatibility {scores['lualatex']:.2f}; "
