...
from lib.alkahest.publication_metadata import (
    ContractError,
    validate_record,
    validate_repository,
    validate_schema,
)
from lib.alkahest.metadata_generation import quarto_metadata


ROOT = Path(__file__).resolve().parent.parent
RECORD = json.loads((ROOT / "book/publication.json").read_text(encoding="utf-8"))
SCHEMA = json.loads(
    (ROOT / "config/metadata/publication.schema.json").read_text(encoding="utf-8")
)
...
def integration_root(directory):
    ...
    (root / "scripts/check-source.py").write_text(
        '("publication-metadata", "check-publication-metadata.py", False)\n',
        encoding="utf-8",
    )
    (root / "docs/publication-metadata.md").write_text(
        "book/publication.json config/metadata/publication.schema.json "
        "work-level manifestation\n",
        encoding="utf-8",
    )
    return root
...
def main():
    ...
    expect_failure(
        ...
        lambda: validate_schema(
            changed(
                SCHEMA,
                lambda schema: schema["$defs"]["contributor"]["properties"]["roles"][
                    "items"
                ]["enum"].append("unknown-role"),
            )
        ),
    )
    cases = [
        ...
        (
            ...
            "contributor ids",
            lambda record: record["contributors"].append(
                copy.deepcopy(record["contributors"][0])
            ),
        ),
        (
            "missing-author-role",
        ...
        (
            ...
            "imprint requires",
            lambda record: record["publication"]["imprint"].update(
                {"name": "Example imprint"}
            ),
        ),
        (
            "selected-license-without-expression",
            "needs an SPDX expression",
            lambda record: record["rights"]["licenses"][0].update(
                {"status": "selected"}
            ),
        ),
        (
            ...
            "cannot be combined",
            lambda record: record["accessibility"].update(
        ...
        (
            ...
            lambda record: record["provenance"]["repository"].update(
                {"visibility": "public"}
            ),
        ),
        (
            ...
            lambda record: record["provenance"]["policies"].update(
                {"rights": "../assets.json"}
            ),
        ),
    ]
