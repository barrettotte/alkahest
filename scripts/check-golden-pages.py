...
from lib.alkahest.golden_pages import (
    GoldenPageError,
    baseline_name,
    compare_pixels,
    load_policy,
...
def validate_integration(root, policy):
    paths = {
        ...
        "wrapper": root / "scripts/check-golden-pages.sh",
    }
    try:
    ...
    if 'check-golden-pages.sh"' not in texts["ci"]:
        fail("CI does not run the golden-page artifact gate")
    if '"golden-pages", "check-golden-pages.py"' not in texts["dispatcher"]:
        fail("source dispatcher does not run the golden-page policy")
    if "ALKAHEST_GOLDEN_IN_CONTAINER" not in texts["wrapper"]:
        fail("golden-page wrapper does not enforce the locked container")
    image = policy["rasterizer"]["version"]
    if image not in texts["wrapper"]:
        fail("golden-page wrapper does not document the pinned rasterizer version")
...
def run_artifacts(root, policy, update):
    ...
    if failures:
        fail(
            f"{len(failures)} golden pages changed; review "
            "book/_build/qa/golden-pages/report.md"
        )
