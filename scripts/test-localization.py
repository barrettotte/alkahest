...
SOURCE_CHECKER = ROOT / "scripts" / "check-localization.py"
RENDERED_CHECKER = ROOT / "scripts" / "check-rendered-localization.py"
LANGUAGES = (
    ("en-US", "ltr", "English"),
    ("fr-FR", "ltr", "Français"),
    ("de-DE", "ltr", "Deutsch"),
    ...
    return {
        ...
        "locales": [
            ...
            },
            {
                ...
                "rendered_reference_html": (
                    "book/_build/locale/fr/html/reference.html"
                ),
        ...
        "source_contract": {
            ...
            "toolchain_report": "scripts/toolchain-report.sh",
        },
        "rendered_contract": {
...
def scopes():
    return "\n".join(
        f'<span lang="{tag}"{f" dir=\"{direction}\"" if direction == "rtl" else ""}>'
        f"{text}</span>"
...
def create_fixture(root):
    ...
    write(root / "scripts/toolchain-report.sh", package_text)
...
def source_cases():
    return (
        ...
        (
            ...
            lambda root, _: mutate_file(
                root, "book/chapter.qmd", 'lang="el-GR"', 'lang="el-CY"'
            ),
        ...
        (
            "unsupported CJK",
            lambda root, _: mutate_file(
                root, "book/chapter.qmd", "Русский", "Русский 漢"
            ),
        ...
        (
            "mismatched script",
            lambda root, _: mutate_file(
                root, "book/chapter.qmd", "Русский", "Ελληνικά"
            ),
        ...
        (
            ...
            lambda root, _: (
                ...
                mutate_file(
                    root, "scripts/toolchain-report.sh", "babel-hebrew", "missing"
                ),
            ),
...
def rendered_cases():
    return (
        ...
        (
            ...
            lambda root, _: mutate_file(
                root, "book/_build/html/languages.html", ' dir="rtl"', ""
            ),
            "needs dir=rtl",
        ),
        (
...
def exercise_valid_translation():
    with tempfile.TemporaryDirectory(prefix="alkahest-localization-") as temporary:
        ...
        if result.returncode:
            raise AssertionError(
                f"valid translated locale failed:\n{result.stderr}{result.stdout}"
            )
    return 1
...
def main():
    ...
    print(
        "ok: localization fixtures "
        f"({source_count} source; {rendered_count} rendered)"
    )
    return 0
