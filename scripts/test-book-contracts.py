...
RECORDS = {
    domain["id"]: json.loads((ROOT / domain["record"]).read_text())
    for domain in POLICY["domains"]
}
SCHEMAS = {
    domain["id"]: json.loads((ROOT / domain["schema"]).read_text())
    for domain in POLICY["domains"]
}
...
def main():
    ...
    print(
        ...
        "(7 domains and 11 inventory, layering, schema, and record failures rejected)"
    )
