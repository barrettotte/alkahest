...
def validate_cover_document(policy, records, publication):
    ...
    printer = _exact(
        template["printer_template"], {"id", "revision", "vendor"}, "printer template"
    )
    if not isinstance(printer["id"], str) or ID.fullmatch(printer["id"]) is None:
        fail("printer template id must be lowercase kebab-case")
    if not isinstance(printer["revision"], int) or isinstance(
        printer["revision"], bool
    ...
    for profile in profiles:
        ...
        if barcode_width + 2 * clearance >= trim_width or barcode_height + 2 * clearance >= trim_height:
...
def load_cover_policy(root):
    ...
    for marker in ("check-covers:", "test-covers:", "generate-covers:", "check-cover-artifacts:"):
        if marker not in texts["makefile"]:
            fail(f"Makefile is missing cover target {marker}")
    for marker in (
        '"covers", "check-covers.py"',
        '"covers", "test-covers.py"',
    ):
        if marker not in texts["dispatcher"]:
            fail(f"source dispatcher is missing cover entry {marker}")
    for marker in ("scripts/generate-covers.py", "scripts/check-cover-artifacts.py"):
...
def expected_cover_outputs(root, interior_facts=None):
    ...
    for profile in context["profiles"]:
        ...
        if interior_facts is None:
            ...
            if (
                not isinstance(facts["sha256"], str)
                or SHA256.fullmatch(facts["sha256"]) is None
            ):
...
def check_cover_artifacts(root):
    ...
    manifests = [
        json.loads(files["cover-manifest.json"])
