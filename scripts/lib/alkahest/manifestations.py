...
def validate_repository(root, registry, records):
    ...
    for marker in (
        ...
        if marker not in render:
            fail(f"render wrapper is missing manifestation artifact {marker}")
    integration = {
    ...
    for marker in ("check-manifestations:", "test-manifestations:"):
        if marker not in texts["makefile"]:
            fail(f"Makefile is missing manifestation target {marker}")
    if '"manifestations", "check-manifestations.py"' not in texts["dispatcher"]:
        fail("source dispatcher does not include manifestations")
    for marker in (
