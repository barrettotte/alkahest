...
def _minimal_book_toml(options):
    quote = _yaml_string
    return f"""# This is the only configuration file most authors need to edit.
schema_version = 1
...
created = {quote(options['created'])}
identifier = {quote(options['epub_identifier'])}
excerpt_identifier = {quote(options['preview_identifier'])}
...
make build                           # Build every full-book format.
...
.PHONY: help chapter draft check build excerpt clean
...
build: ## Build full HTML, EPUB, Typst, and LuaLaTeX editions.
	$(ALK) build
...
The same manuscript builds as HTML, EPUB, Typst PDF, and LuaLaTeX PDF.
""",
