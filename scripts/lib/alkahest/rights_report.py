...
def validate_integration(root):
    ...
    files = {
        ...
        "publication": root / "scripts/check-publication.sh",
    ...
    if '"rights-report", "test-rights-report.py"' not in texts["dispatcher"]:
        fail("source dispatcher is missing rights-report fixtures")
    for marker in (
        "generate-rights-report.py",
        "check-rights-report.py",
    ):
        if marker not in texts["ci"]:
            fail(f"CI is missing rights-report command {marker}")
    if "scripts/check-rights-report.py" not in texts["publication"]:
