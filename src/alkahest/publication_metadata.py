...
def validate_contributors(contributors):
    ...
    for contributor in contributors:
        ...
        if (
            ...
            or not roles
            or any(role not in CONTRIBUTOR_ROLES for role in roles)
        ):
...
def validate_accessibility(accessibility):
    exact_object(
        accessibility, {"features", "hazards", "summary", "review"}, "accessibility metadata"
    )
...
def validate_provenance(root, provenance):
    ...
    web_url(
        repository["url"], "source repository URL", required=repository["visibility"] == "public"
    )
    policies = exact_object(
        provenance["policies"],
        {"reproducibility", "rights", "accessibility"},
        "provenance policies",
    )
    return {name: safe_policy_path(root, path, f"{name} policy") for name, path in policies.items()}
...
def validate_repository(root, record, policies):
    ...
    if any(re.search(pattern, quarto, re.MULTILINE) for pattern in forbidden):
...
def load_and_validate(root):
    root = Path(root)
    schema = load_json(
        root / "config/metadata/publication.schema.json", "publication metadata schema"
    )
