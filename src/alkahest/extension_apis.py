...
DIRECT_GENERATOR_MODULES = {
    "covers": "src/alkahest/covers.py",
    "publication-metadata": "src/alkahest/metadata_generation.py",
    "rights-report": "src/alkahest/rights_report.py",
}
...
def _path(value, label):
    ...
    if path.is_absolute() or ".." in path.parts or str(path) != value:
        fail(f"{label} must be a normalized relative path")
    if path.parts[0] not in {"book", "config", "docs", "scripts", "src"}:
...
def validate_extension_apis(root, document=None):
    ...
    if (
        not isinstance(document["api_version"], str)
        or re.fullmatch(
    ...
    if document["stability"] != "provisional":
        fail("extension API stability must remain provisional before the first public release")
    ...
    for position, entry in enumerate(entries):
        ...
        if api_id == "generators":
            declared_generator_tasks = _string_array(
                entry["tasks"], "extension API generator tasks"
            )
    ...
    covered_manifests = {path for path in all_entrypoints if path.endswith("/_extension.yml")}
    if manifests != covered_manifests:
        fail("extension API inventory does not exactly cover bundled extension manifests")
    filters = {path.relative_to(root).as_posix() for path in (root / "book/filters").glob("*.lua")}
    ...
    generator_tasks = {
        name
        for name, task in GENERATORS.items()
        if task.script.startswith("@alkahest.generators.")
        or name in DIRECT_GENERATOR_MODULES
    }
    if set(declared_generator_tasks) != generator_tasks:
        fail("generator API does not exactly cover deterministic generator tasks")
    generator_entrypoints = {
        (
            DIRECT_GENERATOR_MODULES[name]
            if name in DIRECT_GENERATOR_MODULES
            else "src/" + GENERATORS[name].script.removeprefix("@").replace(".", "/") + ".py"
        )
        for name in generator_tasks
    }
    if set(by_id["generators"]["entrypoints"]) != generator_entrypoints:
        fail("generator API does not exactly cover deterministic generator implementations")
    return {
        ...
        "filters": len(filters),
        "generators": len(generator_tasks),
    }
