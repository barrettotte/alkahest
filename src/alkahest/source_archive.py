...
def _validate_prior_editions(root, work_id):
    ...
    for record in editions:
        ...
        if (
            not isinstance(record["edition_statement"], str)
            or not record["edition_statement"].strip()
        ):
...
def _validate_integration(root):
    ...
    for marker in (
        '"source-archive", ":check-source-archive-policy"',
        '"source-archive", "test-source-archive.py"',
        '"source-archive", ":package-source-archive"',
        '"source-archive", ":check-source-archive"',
    ):
        if marker not in texts["tasks"]:
            fail(f"task registry is missing source-archive entry {marker}")
    for marker in ("alkahest package source-archive", "alkahest check source-archive"):
...
def source_archive_members(root):
    ...
    checksum_lines = [f"{_sha256(content)}  {path}" for path, content in sorted(members.items())]
...
def inspect_archive(filename, content, context, expected_members):
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            ...
            manifest = json.loads(relative[f"{PACKAGE_METADATA_ROOT}/MANIFEST.json"])
...
def _restoration_smoke(relative, context):
    with tempfile.TemporaryDirectory(prefix="alkahest-source-restore.") as temporary:
        ...
        make_result = subprocess.run(  # noqa: S603,S607 - validated recovery target
            [  # noqa: S607 - restoration requires Make on PATH
                "make",
                context["policy"]["restoration"]["make_target"],
            ],
            cwd=restore_root,
            check=False,
            capture_output=True,
        ...
        environment["PYTHONPATH"] = str(restore_root / "src")
        source_result = subprocess.run(  # noqa: S603 - fixed interpreter and checker
            command,
            cwd=restore_root,
            env=environment,
            check=False,
