...
def validate_runtime_policy(policy):
    ...
    for bundle in bundles:
        ...
        if not isinstance(bundle.get("credit_text"), str) or not bundle["credit_text"].strip():
            fail(f"runtime bundle {identifier} needs credit_text")
        if bundle["kind"] == "generated-runtime":
            if (
                not isinstance(bundle.get("license_evidence"), str)
                or not bundle["license_evidence"]
            ):
...
def validate_source_assets(root, policy_path):
    """Validate rights coverage, checksums, privacy, and embedded source metadata."""
...
def forbidden_patterns(contract):
    ...
    for record in patterns:
        ...
        try:
            compiled.append((record["label"], re.compile(record["pattern"], re.IGNORECASE)))
...
def validate_release_assets(root, policy_path):
    """Validate rights, privacy, and metadata in every rendered format."""
    root = Path(root).resolve()
    policy_path = Path(policy_path)
    if not policy_path.is_absolute():
        policy_path = root / policy_path
    ...
    return {
        ...
        "pdfs": check_pdfs(root, policy),
        "runtime_bundles": counts["runtime_bundles"],
    }
