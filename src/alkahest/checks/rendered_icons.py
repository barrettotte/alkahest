    ...
    for argument in sys.argv[1:]:
        ...
        for opening, body, following in pattern.findall(markup):
            ...
            if not re.search(r'\bdata-icon="[a-z][a-z0-9-]*"', opening):
                raise RuntimeError(
                    f"error: {path}: rendered semantic icon has no canonical identity"
                )
            ...
            if not any(character.isalnum() for character in visible):
                raise RuntimeError(
                    f"error: {path}: rendered semantic icon is not followed by visible text"
                )
        if not declared:
            raise RuntimeError(f"error: {path}: no rendered semantic icons were found")
        if checked != declared:
            raise RuntimeError(
                f"error: {path}: checked {checked} of {declared} rendered semantic icons"
            )
