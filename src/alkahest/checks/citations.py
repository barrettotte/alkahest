...
def main():
    ...
    styles = {
        "citations/chicago-author-date.csl": (
            "91fa1fe9787e737dff0c15d7cf8254c9f2bab4ebb4dccf4553a1f991ebddb7d1",
            "author-date",
            "Chicago Manual of Style 17th edition (author-date)",
        ),
        "citations/ieee.csl": (
            "b4c7619fc16c45a31e4cc3271eab94ffe83192d3b4c7fc729470a3b459448de3",
            "numeric",
    ...
    for relative in sorted(styles):
        ...
        if hashlib.sha256(content).hexdigest() != digest:
            fail(
                f"citation style {relative} changed; review its provenance and update the locked hash"
            )
    ...
    for line_number, line in enumerate(
        ...
        if key in keys:
            fail(
                f"{bibliography_path}:{line_number}: duplicate bibliography key {key}; first declared at line {key_lines[key]}"
            )
        keys.add(key)
        ...
        for line_number, line in enumerate(source.read_text(encoding="utf-8").splitlines(), 1):
            ...
            for key in re.findall(r"(?<![A-Za-z0-9_])@([A-Za-z0-9][A-Za-z0-9_.:+-]*)", line):
                if key in keys:
                    ...
                    fail(
                        f"{source}:{line_number}: citation references missing bibliography key {key}"
                    )
    for key in sorted(keys):
        if key not in cited and key not in nocite:
            fail(
                f"bibliography key {key} is unused; cite it or list it explicitly in nocite metadata"
            )
    print(
        f"ok: citations ({len(keys)} bibliography keys; {len(cited)} cited; {len(nocite)} explicit nocite; {len(styles)} locked styles; shared Pandoc citeproc path)"
    )
