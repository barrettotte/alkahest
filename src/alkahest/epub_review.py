...
def bind_review_artifact(root, review_path, source_revision, prepared_at):
    """Bind a wholly pending review ledger to one rendered EPUB and revision."""
    ...
    if set(statuses) != {"pending"}:
        fail("cannot replace artifact identity after manual evidence is recorded")
    artifact = review.get("artifact", {})
    artifact_path = root_path(root, artifact.get("path"), "review artifact path")
    if not artifact_path.is_file():
        fail(f"missing rendered EPUB: {artifact_path}")
    artifact.update(
