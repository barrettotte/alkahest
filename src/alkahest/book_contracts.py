...
EXPECTED_VALIDATOR_TASKS = {
    "stable-ids": "identities",
    "edition-manifests": "editions",
    "publishing-metadata": "publication-metadata",
    "rights-records": "asset-rights",
    "accessibility-metadata": "epub-accessibility",
    "cover-parameters": "covers",
    "localized-labels": "localization",
}
...
def validate_book_contracts(root, document=None, records=None, schemas=None):
    ...
    for domain in domains:
        ...
        validator_task = domain["validator_task"]
        if validator_task != EXPECTED_VALIDATOR_TASKS[domain_id]:
            fail(f"book-contract {domain_id} validator task is inconsistent")
        ...
        for marker in (
            ...
            domain["schema"],
            domain["record"],
            f"alkahest check {validator_task}",
        ):
